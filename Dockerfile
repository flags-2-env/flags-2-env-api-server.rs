...
#
# Multi-stage image for flags-2-env-api-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t flags-2-env-api-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" flags-2-env-api-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=flags-2-env-api-server-target-${TARGETARCH},sharing=locked \
...
COPY --from=build "/usr/local/bin/flags-2-env-api-server" "/usr/local/bin/flags-2-env-api-server"
...
RUN --mount=type=bind,source=.,target=/src,ro \
    ...
    && chown -R 65532:65532 /app /usr/local/bin/flags-2-env-api-server
ENV SOPS_SECRETS_FILE=/app/secrets/app.env \
    OTEL_SERVICE_NAME=flags-2-env-api-server \
