...
#
# flags-2-env-api-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/flags-2-env/flags-2-env-api-server:dev --load .
#
...
#     -t ghcr.io/flags-2-env/flags-2-env-api-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/flags-2-env/flags-2-env-api-server:dev .
#
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/flags-2-env-api-server /usr/local/bin/flags-2-env-api-server
ENV FLAGS_2_ENV_API_BIND=0.0.0.0:8080
EXPOSE 8080
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/flags-2-env-api-server"]
...
COPY --from=builder /usr/local/bin/flags-2-env-api-server /usr/local/bin/flags-2-env-api-server
...
#     ghcr.io/flags-2-env/flags-2-env-api-server:dev
#
...
USER 10001:10001
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/flags-2-env-api-server"]
CMD []
